...
namespace SSHInfo.Controllers
{
    [Route("api/[controller]")]
    [ApiController]
    public class SSHWebAPIController : ControllerBase
    {
        private readonly SSHDbContext _sshontext;

        public SSHWebAPIController(SSHDbContext sshontext)
        {
            _sshontext = sshontext;
        }
        ...
        public IActionResult CheckVirtualMachineStatus(string host) 
        {
            try
            {
                ...
                var virtualMachine = _sshontext.SSHInfos.FirstOrDefault(vm => vm.Host == host);
        ...
        private bool IsVirtualMachineRunning(SSHInfo.Classes.SSHInfo virtualMachine)
        {
            try
            {
        ...
        public IActionResult GetAllVirtualMachines()
        {
            try
            {
                var virtualMachines = _sshontext.SSHInfos.ToList();
        ...
        private void OpenVirtualMachine(SSHInfo.Classes.SSHInfo virtualMachine)
        {
            string arguments = $"startvm \"{virtualMachine.VirtualName}\"";
            ExecuteVBoxManageCommand(arguments);
        }

        private void CloseVirtualMachine(SSHInfo.Classes.SSHInfo virtualMachine)
        {
        ...
        private bool IsDockerInstalled(Classes.SSHInfo virtualMachine)
        ...
        public IActionResult GetMemoryUsage(string host)
        {
            try
            {
                ...
                var virtualMachine = _sshontext.SSHInfos.FirstOrDefault(x => x.Host == host);
                if (virtualMachine != null)
                {
        ...
        private string ExecuteCommand(SSHInfo.Classes.SSHInfo virtualMachine, string command)
        {
            try
            {
